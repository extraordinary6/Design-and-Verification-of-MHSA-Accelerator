...
Wo = np.random.randint(-128, 127, size=(128, 128), dtype=np.int8)
...
# -------------------connect layer------------------
attention_concat = np.concatenate(attention_quantized, axis=1)
connect_ref = np.dot(attention_concat.astype(np.int32), Wo.astype(np.int32))

# 量化
connect_quantized = connect_ref.astype(np.int8)
...
#--------------------------connect层验证----------------------------
connect_quantized_t = connect_quantized.T
connect_output = []

for i in range(0, connect_quantized_t.size, 8):  # 每次取 8 个元素
    chunk = connect_quantized_t.flatten()[i:i+8]
    if len(chunk) < 8:
        break
    bytes_data = chunk.astype(np.uint8).tobytes()  # 转换为无符号字节
    value = int.from_bytes(bytes_data, byteorder='big')
    hex_str = f"{value:016X}"  # 格式化为 16 位宽的十六进制字符串
    connect_output.append(hex_str)
with open('connect_output.txt', 'w') as f:
    for i in range(0, len(connect_output), 4):
        row = connect_output[i:i+4]
        f.write(' '.join(row) + '\n')
