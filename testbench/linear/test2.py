...
B = np.random.randint(-128, 127, size=(128, 32), dtype=np.int8)
...
hex_output = []
for i in range(0, C_quantized.size, 8):  # 每次取 8 个元素
    chunk = C_quantized.flatten()[i:i+8]  # 取出 8 个元素
    if len(chunk) < 8:
